...
static inline cache_t *create_cache(const char *trace_path,
  ...
  if (strcasecmp(eviction_algo, "lru") == 0) {
  ...
  } else if (strcasecmp(eviction_algo, "s3fifod3") == 0) {
    cache = S3FIFOd3_init(cc_params, eviction_params);
    ...
    cache = S4FIFO_init(cc_params, eviction_params);
  } else if (strcasecmp(eviction_algo, "s3lfu") == 0) {
    cache = S3LFU_init(cc_params, eviction_params);
  } else if (strcasecmp(eviction_algo, "s3fifo_time_delay") == 0){
    cache = S3FIFO_TIME_DELAY_init(cc_params, eviction_params);
