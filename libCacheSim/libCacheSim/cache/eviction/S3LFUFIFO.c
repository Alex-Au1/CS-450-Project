//
//  10% small LFU + 90% main FIFO (2-bit Clock) + ghost
//  insert to small LFU if not in the ghost, else insert to the main FIFO
...
static const char *DEFAULT_CACHE_PARAMS =
    ...
    "type=fifo,move-to-main-threshold=1";
...
// ****                                                               ****
// ***********************************************************************
cache_t *S3LFUFIFO_init(const common_cache_params_t ccache_params,
                    const char *cache_specific_params);
static void S3LFUFIFO_free(cache_t *cache);
static bool S3LFUFIFO_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3LFUFIFO_find(cache_t *cache, const request_t *req,
                               const bool update_cache);
static cache_obj_t *S3LFUFIFO_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3LFUFIFO_to_evict(cache_t *cache, const request_t *req);
static void S3LFUFIFO_evict(cache_t *cache, const request_t *req);
static bool S3LFUFIFO_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3LFUFIFO_get_occupied_byte(const cache_t *cache);
static inline int64_t S3LFUFIFO_get_n_obj(const cache_t *cache);
static inline bool S3LFUFIFO_can_insert(cache_t *cache, const request_t *req);
static void S3LFUFIFO_parse_params(cache_t *cache,
                               const char *cache_specific_params);

static void S3LFUFIFO_evict_LFU(cache_t *cache, const request_t *req);
static void S3LFUFIFO_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3LFUFIFO_init(const common_cache_params_t ccache_params,
                    const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S3LFUFIFO", ccache_params, cache_specific_params);
  cache->cache_init = S3LFUFIFO_init;
  cache->cache_free = S3LFUFIFO_free;
  cache->get = S3LFUFIFO_get;
  cache->find = S3LFUFIFO_find;
  cache->insert = S3LFUFIFO_insert;
  cache->evict = S3LFUFIFO_evict;
  cache->remove = S3LFUFIFO_remove;
  cache->to_evict = S3LFUFIFO_to_evict;
  cache->get_n_obj = S3LFUFIFO_get_n_obj;
  cache->get_occupied_byte = S3LFUFIFO_get_occupied_byte;
  cache->can_insert = S3LFUFIFO_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3LFUFIFO_params_t));
  memset(cache->eviction_params, 0, sizeof(S3LFUFIFO_params_t));
  S3LFUFIFO_params_t *params = (S3LFUFIFO_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;
  params->promote_on_hit = true;

  S3LFUFIFO_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3LFUFIFO_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3LFUFIFO-%s-%d-%.4lf-%d",
...
static cache_obj_t *S3LFUFIFO_find(cache_t *cache, const request_t *req,
  ...
  if (obj != NULL) {
    obj->S3LFUFIFO.freq += 1;

    if (params->promote_on_hit &&
        obj->S3LFUFIFO.freq >= params->move_to_main_threshold) {
      // move to main cache
...
static void S3LFUFIFO_evict_LFU(cache_t *cache, const request_t *req) {
  ...
  while (!has_evicted && LFU->get_occupied_byte(LFU) > 0) {
    ...
    if (!params->promote_on_hit &&
        obj_to_evict->S3LFUFIFO.freq >= params->move_to_main_threshold) {
      // move to main cache
#if defined(TRACK_DEMOTION)
...
static void S3LFUFIFO_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    // return S3LFUFIFO_evict_main(cache, req);
    return main->evict(main, req);
  } else {
    return S3LFUFIFO_evict_LFU(cache, req);
  }
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S3LFUFIFO_current_params(S3LFUFIFO_params_t *params) {
...
static void S3LFUFIFO_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "LFU-size-ratio") == 0) {
      ...
      printf("parameters: %s\n", S3LFUFIFO_current_params(params));
      exit(0);
    } else {
