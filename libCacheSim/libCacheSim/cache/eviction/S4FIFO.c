...
static void S4FIFO_evict_fifo(cache_t *cache, const request_t *req);
...
static void S4FIFO_free(cache_t *cache) {
  ...
  if (params->fifo_ghost_2 != NULL) {
    params->fifo_ghost_2->cache_free(params->fifo_ghost_2);
  }
...
static cache_obj_t *S4FIFO_find(cache_t *cache, const request_t *req,
  ...
  if (params->fifo_ghost_2 != NULL &&
        params->fifo_ghost_2->remove(params->fifo_ghost_2, req->obj_id)) {
    // if object in fifo_ghost, remove will return true
    params->hit_on_ghost_2 = true;
}
...
static void S4FIFO_evict_fifo(cache_t *cache, const request_t *req) {
  ...
  cache_t *ghost_2 = params->fifo_ghost_2;
...
static void S4FIFO_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    return S4FIFO_evict_main(cache, req);
  }
  return S4FIFO_evict_fifo(cache, req);
}
...
static bool S4FIFO_remove(cache_t *cache, const obj_id_t obj_id) {
  ...
  removed = removed || (params->fifo_ghost_2 &&
                        params->fifo_ghost_2->remove(params->fifo_ghost_2, obj_id));
